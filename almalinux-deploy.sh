...
distro_sync() {
    ...
    if [ $is_voipnow -eq 1 ]; then
	exclude_pkgs+=",LibRaw*,jansson*,libnghttp2*,libzstd*,opus*"
    fi
...
main() {
	...
	is_voipnow=1
	if [ `rpm -q --queryformat '%{VERSION}' voipnow-core|sed 's/\.//g'` -lt 555 ];then 
		report_step_error "%{step}. VoipNow version must be 5.5.5 or higher"
		exit -1
        fi
    fi
